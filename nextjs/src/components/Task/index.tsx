    ...
    (evt: FormEvent<EventTarget>) => {
      ...
      if (questionType === "radio") {
        ...
        if (target.checked) {
          ...
          currentAnswer.splice(currentAnswer.indexOf(parseInt(answer)), 1);
        }
      } else {
  ...
  return (
    <div className={styles.form} style={customStyles}>
      ...
      {currentQuestion === questions.length ? (
        <>
          <RevisionComponent questions={questions} hits={hits} />
          <button type="button" onClick={closeTask}>
            Receber pontos!
          </button>
        </>
      ) : (
        <div className={styles.buttonContainer}>
          ...
          {currentQuestion === questions.length - 1 ? (
            <button type="button" onClick={sendAnswers}>
              Completar
            </button>
          ) : (
