...
interface AuthContextData {
  ...
  createNewUser: (name: string, number: number, image: string) => void;
}
...
export function AuthProvider({ children }: ChallengeProviderProps) {
  ...
  useEffect(() => {
    ...
    if (name) {
      axios
        .get("/api/users/", { params: { nickname: name } })
        .then((response) => {
  ...
  useEffect(() => {
    ...
    if (
      !cookieNickname ||
      (cookieNickname === nickname && cookieNickname === "Novato(a)")
    ) {
      setIsNewUser(true);
    }
  ...
  function createNewUser(name: string, idade: number, image: string) {
    axios
      .post("/api/users/", { nickname: name, idade, image })
      .then((response) => {
        const user = response.data.user;
        updateUser(name, user);
      });
