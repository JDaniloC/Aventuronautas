...
function compareAnswers(type: string, correct: number[], answer: number[]) {
  ...
  correct.forEach((value, index) => {
    if (type === "select" && value === answer[index]) {
      current += 1;
    } else if (type === "check" && answer.indexOf(value) !== -1) {
      current += 1;
    }
  });
  ...
  for (let i = 0; i < questions.length; i++) {
    ...
    answers.forEach((answer: Answer) => {
      if (question.id === answer.id) {
        if (questionType === "radio" && correct === answer.option) {
          score += 1;
          hits.push(100);
        } else if (questionType !== "radio") {
          const currentScore = compareAnswers(
            questionType,
            correct as number[],
...
export default async (req: VercelRequest, res: VercelResponse) => {
  ...
  if (req.method === "GET") {
    ...
    const questions = await questionsCollection
      .find({ mission }, { projection: { answer: false } })
      .toArray();
    ...
    const questions = await questionsCollection
      .find(
        { mission },
        { projection: { type: true, id: true, options: true, answer: true } },
      )
      .toArray();
